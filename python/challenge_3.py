...
def get_real_polices(polices_db: str) -> List[str]:
    def reduce_real_polices(acc: List[str], police: str) -> List[str]:
        data, password = police.split(':')
        interval, letter = data.split(' ')
        min, max = interval.split('-')
        char_count = password.count(letter)
        if char_count >= int(min) and char_count <= int(max):
            return acc
        acc.append(password.strip())
        return acc
    polices_db = polices_db.split('\n')
    return reduce(reduce_real_polices, polices_db, [])


print(get_real_polices(polices_db)[41])
